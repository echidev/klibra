...
This runbook provides step-by-step procedures for responding to a failed ingestion pipeline. The goal is to detect, diagnose, contain, recover, validate, communicate, and prevent recurrence.
...
3. Note the pipeline run ID
4. Check if the failure is isolated to one dataset or affects multiple
...
Refer to TDD Section 26 failure classification:
...
3. **Alert the relevant stakeholders** — notify Data Owner and Business Owner if P0/P1
4. **Document the failure** — record in incident management system
...
| Source unavailable | Wait for source restoration; check alternate method |
| Authentication failure | Rotate credentials; verify new credentials; rerun |
| Rate limiting | Back off and retry with delay; contact source if persistent |
| Schema change | Evaluate change type (compatible/breaking); adapt connector |
| Parsing error | Fix parser; validate against new payload format |
| Data quality | Quarantine failing records; investigate root cause |
...
3. Rerun with idempotency key to prevent duplication
...
6. **Test resilience** — add failure test if applicable
