...
- Securely isolate environments (PRD §35).
- Cost‑effective for academic/pilot workload (PRD §36).
...
- Secrets never in code (PRD §35). Key rotation via Secrets Manager.
- Cost monitoring via AWS Cost Explorer + budget alerts (PRD §36).
...
- Security checklist (PRD §35) verified for staging and production.
